(() => {
  ...
  const POPULARITY = window.CATEGORY_POPULARITY_V63 || window.CATEGORY_POPULARITY_V62 || window.CATEGORY_POPULARITY_V61 || window.CATEGORY_POPULARITY_V60 || window.CATEGORY_POPULARITY_V59 || { sourceRows: [], aliases: {}, aggregates: {}, matchesById: {} };
  ...
  // V56 — структурные правки раздела «Инструменты».
  ...
  function renderHome() {
    const entries = [
      { kind: "types", name: "Инструменты", image: "assets/generated/phase-01/entry-03-tool-types.webp" },
  ...
  function renderTypes() {
    return `${breadcrumbs([{ name: "Каталог", kind: "home" }, { name: "Ремонт и строительство" }])}
      <section class="page-intro"><h1>Инструменты</h1></section>
  ...
  function renderMacro() {
    ...
    return `${breadcrumbs([{ name: "Каталог", kind: "home" }, { name: "Инструменты", kind: "types" }, { name: section.name }])}
  ...
  function renderGroup() {
    ...
    return `${breadcrumbs([{ name: "Каталог", kind: "home" }, { name: "Инструменты", kind: "types" }, { name: section.name, kind: "macro", id: section.id }, { name: group.name }])}
  ...
  function renderOperationPage(stage, workIndex, operationIndex) {
    ...
    const crumbs = [{ name: "Каталог", kind: "home" }, { name: "Подбор по этапам работ", kind: "stages" }, { name: stage.name, kind: "stage", id: String(stage.id) }];
  ...
  function renderResourceGroup() {
    ...
    const crumbs = [{ name: "Каталог", kind: "home" }, { name: "Подбор по этапам работ", kind: "stages" },
  ...
  function renderWork() {
    ...
    return `${breadcrumbs([{ name: "Каталог", kind: "home" }, { name: "Подбор по этапам работ", kind: "stages" }, { name: stage.name, kind: "stage", id: String(stage.id) }, { name: work.name }])}
  ...
  function renderMegaCatalog() {
    return `<div class="mega-catalog-layout"><div class="mega-directory">
      <section class="mega-path-block"><button type="button" class="mega-path-title" ${navAttrs("types")}><span class="mega-path-number">01</span><span><strong>Инструменты</strong></span></button><div class="mega-level-list">${typeRoots.map((node) => megaLevelItem({ name: node.title, meta: typeNodeMeta(node), navKind: "type-node", navId: node.id, previewKind: "type-node", previewId: node.id })).join("")}</div></section>
  ...
  function renderTree() {
    ...
    modalRoot.innerHTML = `<div class="tree-overlay" data-action="close-tree"><aside class="tree-panel"><header><div><h2>Дерево каталога</h2></div><button type="button" data-action="close-tree" aria-label="Закрыть">×</button></header><div class="tree-content"><details open><summary><strong>Инструменты</strong><small>${typeRoots.length} разделов</small></summary><div>${renderTypeTree()}</div></details><details open><summary><strong>Подбор по этапам работ</strong><small>${visibleStages.length} этапов</small></summary><div>${stageTree}</div></details><details open><summary><strong>Аккумуляторные платформы</strong><small>${availableBrands.length} брендов</small></summary><div>${platformTree}</div></details></div></aside></div>`;
  }
