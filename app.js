(() => {
  ...
  function buildTemplateStages() {
    return [
      templateStage(1, "Проектирование и земляные работы", [
        templateWork("Подготовка и разметка", [
          ["Измерение и разметка", resourceSet(
            [resource("Лазерный нивелир", "Лазерные нивелиры и уровни"), resource("Лазерный дальномер", "Лазерные дальномеры")],
            [resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Маркер", "Маркеры и карандаши строительные")],
            [resource("Мишени для лазерных плоскостей", "Мишени для лазерных плоскостей")],
          )],
          ["Организация рабочего места", resourceSet(
            [resource("Строительный фонарь", "Фонари")],
            [resource("Сумки для инструмента", "Сумки для инструмента"), resource("Ящики для хранения инструмента", "Ящики для хранения инструмента")],
            [],
            [resource("Защитные очки", "Очки защитные"), resource("Защитные перчатки", "Перчатки"), resource("Защитные наушники", "Наушники защитные"), resource("Респиратор", "Респираторы"), resource("Спецодежда", "Спецодежда")],
          )],
          ["Временное электроснабжение", resourceSet(
            [resource("Виброплита", "Виброплиты")],
            [proposedResource("Лопаты и штыковой инструмент"), proposedResource("Строительная тачка"), proposedResource("Временные кабели и удлинители")],
            [proposedResource("Геотекстиль и материалы для основания")],
            [resource("Защитные перчатки", "Перчатки"), resource("Защитные ботинки", "Спецодежда")],
          )],
          ["Освещение и работа на высоте", resourceSet([resource("Строительный фонарь", "Фонари")], [proposedResource("Лестницы и стремянки")], [], [resource("Каска", "Спецодежда")])],
          ["Безопасность и СИЗ", resourceSet([], [], [], [resource("Защитные очки", "Очки защитные"), resource("Защитные перчатки", "Перчатки"), resource("Респиратор", "Респираторы"), resource("Спецодежда", "Спецодежда")])],
          ["Подготовка инструмента", resourceSet([resource("Дрель", "Дрели")], [resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")], [proposedResource("Наборы ручного инструмента")], [])],
        ]),
        templateWork("Устройство фундамента", [
          ["Земляные работы и котлован", resourceSet(
            [resource("Виброплита", "Виброплиты")],
            [proposedResource("Лопаты и штыковой инструмент"), proposedResource("Строительная тачка")],
            [proposedResource("Геотекстиль и материалы для основания")],
            [resource("Защитные перчатки", "Перчатки"), resource("Спецодежда", "Спецодежда")],
          )],
          ["Опалубка и армирование", resourceSet(
            [resource("Дрель", "Дрели")],
            [resource("Струбцина", "Струбцины"), resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Опалубочные щиты"), proposedResource("Арматура и вязальная проволока"), proposedResource("Крепёж для опалубки")],
            [resource("Защитные очки", "Очки защитные"), resource("Защитные перчатки", "Перчатки")],
          )],
          ["Приготовление и укладка бетона", resourceSet(
            [resource("Бетоносмеситель", "Бетоносмесители"), resource("Вибратор для бетона", "Вибраторы для бетона"), resource("Виброрейка", "Виброрейки")],
            [proposedResource("Правило строительное"), resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки")],
            [proposedResource("Бетонная смесь")],
            [resource("Защитные очки", "Очки защитные"), resource("Защитные перчатки", "Перчатки"), resource("Респиратор", "Респираторы")],
          )],
          ["Гидроизоляция фундамента", resourceSet(
            [],
            [resource("Шпатель", "Шпатели"), resource("Кисть", "Кисти малярные")],
            [proposedResource("Гидроизоляционные материалы"), proposedResource("Грунтовка для основания")],
            [resource("Перчатки", "Перчатки"), resource("Респиратор", "Респираторы")],
          )],
        ]),
      ]),
      templateStage(2, "Демонтажные работы", [
        templateWork("Демонтаж отделки и инженерных элементов", [
          ["Снятие штукатурки, плитки и напольных покрытий", resourceSet(
            [resource("Перфоратор", "Перфораторы"), resource("Реноватор", "Реноваторы")],
            [resource("Молоток", "Молотки"), resource("Зубило", "Зубила"), resource("Скребок", "Скребки"), resource("Лом и гвоздодёр", "Ломы, монтировки и гвоздодеры")],
            [resource("Зубила для перфораторов", "Зубила для перфораторов и отбойных молотков"), resource("Насадки для реноватора", "Насадки для реноваторов"), resource("Лезвия", "Лезвия для ножей")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Демонтаж дверей, окон и сантехники", resourceSet(
            [resource("Сабельная пила", "Сабельные пилы"), resource("Перфоратор", "Перфораторы")],
            [resource("Лом и гвоздодёр", "Ломы, монтировки и гвоздодеры"), resource("Клещи", "Клещи"), resource("Ключ разводной", "Ключи разводные")],
            [resource("Полотна для сабельной пилы", "Полотна для сабельных пил")],
            [resource("Защитные очки", "Очки защитные"), resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
        templateWork("Снос стен, перегородок и старых конструкций", [
          ["Разборка стен и перегородок", resourceSet(
            [resource("Отбойный молоток", "Отбойные молотки"), resource("Перфоратор", "Перфораторы")],
            [resource("Кувалда", "Кувалды"), resource("Зубило", "Зубила")],
            [resource("Зубила для перфораторов", "Зубила для перфораторов и отбойных молотков"), resource("Наборы буров и зубил", "Наборы буров и зубил")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы"), resource("Защитные наушники", "Наушники защитные")],
          )],
          ["Демонтаж кровли и фундамента", resourceSet(
            [resource("Отбойный молоток", "Отбойные молотки"), resource("УШМ", "Углошлифовальные машины (УШМ)")],
            [resource("Кувалда", "Кувалды"), resource("Лом и гвоздодёр", "Ломы, монтировки и гвоздодеры")],
            [resource("Алмазные диски", "Алмазные диски"), resource("Отрезные диски", "Отрезные и обдирочные диски")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Резка труб и арматуры", resourceSet(
            [resource("УШМ", "Углошлифовальные машины (УШМ)"), resource("Сабельная пила", "Сабельные пилы")],
            [resource("Болторез", "Болторезы"), resource("Ножницы по металлу", "Ножницы по металлу")],
            [resource("Отрезные диски", "Отрезные и обдирочные диски"), resource("Полотна для сабельной пилы", "Полотна для сабельных пил")],
            [resource("Защитные очки", "Очки защитные"), resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
        templateWork("Вывоз мусора и расчистка", [
          ["Сбор и вывоз строительного мусора", resourceSet(
            [],
            [proposedResource("Строительные тачки"), proposedResource("Лопаты"), proposedResource("Щётки и совки")],
            [proposedResource("Мешки для строительного мусора")],
            [resource("Защитные перчатки", "Перчатки"), resource("Спецодежда", "Спецодежда"), resource("Защитные очки", "Очки защитные")],
          )],
          ["Удаление пыли и мелких остатков", resourceSet(
            [resource("Строительный пылесос", "Строительные пылесосы")],
            [],
            [resource("Мешки для строительного пылесоса", "Мешки для строительных пылесосов"), resource("Фильтры для строительного пылесоса", "Фильтры для строительных пылесосов")],
            [resource("Респиратор", "Респираторы")],
          )],
        ]),
      ]),
      templateStage(3, "Возведение коробки, перегородок, кровли и фасада", [
        templateWork("Стены и перегородки", [
          ["Возведение стен и кладка", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры")],
            [resource("Молоток", "Молотки"), resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки"), proposedResource("Правило строительное")],
            [proposedResource("Кладочные смеси"), proposedResource("Армирующая сетка")],
            [resource("Защитные очки", "Очки защитные"), resource("Защитные перчатки", "Перчатки")],
          )],
          ["Монтаж перегородок и каркасов", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты"), resource("Перфоратор", "Перфораторы")],
            [resource("Ножницы по металлу", "Ножницы по металлу"), resource("Плоскогубцы", "Плоскогубцы и пассатижи"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [resource("Буры", "Буры"), resource("Лезвия", "Лезвия для ножей"), proposedResource("Профили и крепёж для ГКЛ")],
            [resource("Защитные очки", "Очки защитные")],
          )],
          ["Монтаж перекрытий", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты"), resource("Сабельная пила", "Сабельные пилы")],
            [resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Молоток", "Молотки")],
            [resource("Полотна для сабельной пилы", "Полотна для сабельных пил"), proposedResource("Крепёж для перекрытий")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
        templateWork("Кровельные работы", [
          ["Монтаж стропильной системы", resourceSet(
            [resource("Сабельная пила", "Сабельные пилы"), resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Молоток", "Молотки"), resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [resource("Полотна для сабельной пилы", "Полотна для сабельных пил"), proposedResource("Пиломатериалы и кровельный крепёж")],
            [resource("Защитные перчатки", "Перчатки"), resource("Страховочная экипировка", "Спецодежда")],
          )],
          ["Утепление и укладка кровельного покрытия", resourceSet(
            [resource("Электрический степлер", "Степлеры электрические")],
            [resource("Нож строительный", "Специализированные ножи"), resource("Ножницы по металлу", "Ножницы по металлу")],
            [resource("Скобы", "Скобы для пистолетов"), proposedResource("Утеплитель и кровельное покрытие")],
            [resource("Защитные перчатки", "Перчатки"), resource("Страховочная экипировка", "Спецодежда")],
          )],
        ]),
        templateWork("Фасадные работы", [
          ["Утепление фасада", resourceSet(
            [resource("Перфоратор", "Перфораторы"), resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Нож строительный", "Специализированные ножи"), resource("Молоток", "Молотки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Фасадные дюбели"), proposedResource("Клеевые составы для утеплителя")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Облицовка фасада", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Ножницы по металлу", "Ножницы по металлу"), resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Фасадные панели и крепёж")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
      ]),
      templateStage(4, "Черновая инженерия — Электрика и сантехника", [
        templateWork("Электромонтаж", [
          ["Штробление и подготовка трасс", resourceSet(
            [resource("Штроборез", "Штроборезы"), resource("Перфоратор", "Перфораторы")],
            [resource("Молоток", "Молотки"), resource("Зубило", "Зубила"), resource("Маркер", "Маркеры и карандаши строительные")],
            [resource("Алмазные диски", "Алмазные диски"), resource("Буры", "Буры")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Прокладка кабелей и гофры", resourceSet(
            [],
            [resource("Нож монтерский", "Ножи монтерские"), resource("Кабелерез", "Кабелерезы, тросорезы"), resource("Стриппер", "Стрипперы"), resource("Плоскогубцы", "Плоскогубцы и пассатижи"), proposedResource("Устройство для затяжки кабеля")],
            [proposedResource("Кабель и гофра")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
          ["Установка подрозетников и электрощита", resourceSet(
            [resource("Перфоратор", "Перфораторы"), resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Отвёртка", "Отвертки"), resource("Пробник напряжения", "Пробники напряжения"), resource("Пресс-клещи", "Пресс-клещи (кримперы)")],
            [resource("Коронки", "Коронки"), resource("Буры", "Буры"), resource("Наборы бит", "Наборы бит")],
            [resource("Диэлектрические перчатки", "Перчатки")],
          )],
        ]),
        templateWork("Сантехника и отопление", [
          ["Монтаж труб и канализации", resourceSet(
            [],
            [resource("Труборез", "Труборезы"), resource("Ключ разводной", "Ключи разводные"), resource("Плоскогубцы", "Плоскогубцы и пассатижи")],
            [proposedResource("Трубы, фитинги и канализационные элементы")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
          ["Монтаж отопления и котельной", resourceSet(
            [resource("Дрель", "Дрели"), resource("Перфоратор", "Перфораторы")],
            [resource("Ключ разводной", "Ключи разводные"), resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Радиаторы и котельное оборудование"), proposedResource("Крепёж для инженерных систем")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
          ["Опрессовка и проверка системы", resourceSet(
            [],
            [resource("Ключ разводной", "Ключи разводные"), proposedResource("Манометр")],
            [proposedResource("Опрессовщик"), proposedResource("Шланги для опрессовки")],
            [resource("Защитные перчатки", "Перчатки"), resource("Защитные очки", "Очки защитные")],
          )],
        ]),
        templateWork("Вентиляция и наружные сети", [
          ["Монтаж вентиляции", resourceSet(
            [resource("Дрель", "Дрели"), resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Ножницы по металлу", "Ножницы по металлу"), resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Вентиляционные каналы и решётки"), resource("Наборы бит", "Наборы бит")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
          ["Ввод воды, септик, газ и насосная станция", resourceSet(
            [resource("Дрель", "Дрели"), resource("Перфоратор", "Перфораторы")],
            [resource("Труборез", "Труборезы"), resource("Ключ разводной", "Ключи разводные")],
            [proposedResource("Насосные станции"), proposedResource("Септики и наружная канализация"), proposedResource("Газовое оборудование")],
            [resource("Защитные перчатки", "Перчатки"), resource("Защитные очки", "Очки защитные")],
          )],
        ]),
      ]),
      templateStage(5, "Черновая отделка — Штукатурка и стяжка", [
        templateWork("Штукатурка стен и потолков", [
          ["Штукатурка стен по маякам", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры")],
            [resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки"), resource("Шпатель", "Шпатели"), proposedResource("Правило строительное"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Штукатурные смеси"), proposedResource("Маяки для штукатурки")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Выравнивание потолков", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры")],
            [resource("Шпатель", "Шпатели"), proposedResource("Правило строительное"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Штукатурные и выравнивающие смеси")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
        ]),
        templateWork("Стяжка и наливной пол", [
          ["Устройство стяжки пола", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры"), resource("Виброрейка", "Виброрейки"), resource("Вибратор для бетона", "Вибраторы для бетона")],
            [proposedResource("Правило строительное"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Сухие смеси для стяжки"), proposedResource("Маяки для стяжки")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Гидроизоляция мокрых зон", resourceSet(
            [],
            [resource("Шпатель", "Шпатели"), resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки")],
            [proposedResource("Гидроизоляционные составы и ленты"), proposedResource("Грунтовка")],
            [resource("Перчатки", "Перчатки"), resource("Респиратор", "Респираторы")],
          )],
          ["Сушка и подготовка основания", resourceSet(
            [resource("Строительный пылесос", "Строительные пылесосы")],
            [resource("Скребок", "Скребки")],
            [resource("Фильтры для строительного пылесоса", "Фильтры для строительных пылесосов"), resource("Мешки для строительного пылесоса", "Мешки для строительных пылесосов")],
            [resource("Респиратор", "Респираторы")],
          )],
        ]),
      ]),
      templateStage(6, "Плиточные работы", [
        templateWork("Разметка, раскрой и сверление плитки", [
          ["Раскладка и разметка плитки", resourceSet(
            [],
            [resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Маркер", "Маркеры и карандаши строительные")],
            [proposedResource("Плиточные шаблоны")],
            [resource("Защитные очки", "Очки защитные")],
          )],
          ["Раскрой плитки", resourceSet(
            [proposedResource("Плиткорез"), resource("УШМ", "Углошлифовальные машины (УШМ)")],
            [resource("Струбцина", "Струбцины")],
            [resource("Алмазные диски", "Алмазные диски")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Сверление плитки", resourceSet(
            [resource("Дрель", "Дрели")],
            [proposedResource("Шаблоны для сверления")],
            [resource("Коронки", "Коронки"), resource("Сверла", "Сверла")],
            [resource("Защитные очки", "Очки защитные")],
          )],
        ]),
        templateWork("Укладка и затирка", [
          ["Укладка плитки на стены, пол и террасы", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры")],
            [resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки"), resource("Шпатель", "Шпатели"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Плиточный клей"), proposedResource("Система выравнивания плитки")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
          ["Затирка швов", resourceSet(
            [],
            [resource("Шпатель", "Шпатели"), proposedResource("Губка для затирки"), resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки")],
            [proposedResource("Затирочные составы")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
      ]),
      templateStage(7, "Предчистовая подготовка - Шпаклёвка и шлифовка", [
        templateWork("Шпаклёвка стен и потолков", [
          ["Шпаклёвка стен", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры")],
            [resource("Шпатели", "Шпатели"), resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки"), resource("Скребки", "Скребки")],
            [proposedResource("Шпаклёвочные составы")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
          ["Шпаклёвка потолков", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры")],
            [resource("Шпатели", "Шпатели"), proposedResource("Правило строительное")],
            [proposedResource("Шпаклёвочные составы для потолка")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
        ]),
        templateWork("Шлифовка и обеспыливание", [
          ["Шлифовка стен и потолков", resourceSet(
            [resource("Шлифовальная машина для стен и потолка", "Шлифовальные машины для стен и потолка"), resource("Виброшлифовальная машина", "Виброшлифовальные машины")],
            [resource("Губки шлифовальные", "Губки шлифовальные")],
            [resource("Шлифовальные листы", "Шлифовальные листы")],
            [resource("Респиратор", "Респираторы"), resource("Защитные очки", "Очки защитные")],
          )],
          ["Шлифовка углов и откосов", resourceSet(
            [resource("Виброшлифовальная машина", "Виброшлифовальные машины")],
            [proposedResource("Шлифовальная губка"), resource("Шпатели", "Шпатели")],
            [resource("Шлифовальные листы", "Шлифовальные листы")],
            [resource("Респиратор", "Респираторы")],
          )],
          ["Обеспыливание", resourceSet(
            [resource("Строительный пылесос", "Строительные пылесосы")],
            [],
            [resource("Мешки для строительного пылесоса", "Мешки для строительных пылесосов"), resource("Фильтры для строительного пылесоса", "Фильтры для строительных пылесосов")],
            [resource("Респиратор", "Респираторы")],
        ...
        templateWork("Подготовка пола", [
          ["Наливной пол", resourceSet(
            [resource("Дрель-миксер", "Дрели-миксеры")],
            [proposedResource("Правило строительное"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Наливные смеси")],
            [resource("Защитные очки", "Очки защитные"), resource("Респиратор", "Респираторы")],
          )],
        ]),
      ]),
      templateStage(8, "Чистовая отделка потолка и стен", [
        templateWork("Потолки и оконные откосы", [
          ["Монтаж натяжного потолка и ГКЛ", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты"), resource("Перфоратор", "Перфораторы"), resource("Лобзик", "Лобзики электрические")],
            [resource("Нож строительный", "Специализированные ножи"), resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [resource("Коронки", "Коронки"), resource("Буры", "Буры"), resource("Наборы бит", "Наборы бит"), proposedResource("Комплектующие для натяжных потолков и профили ГКЛ")],
            [resource("Защитные очки", "Очки защитные")],
          )],
          ["Установка подоконников и откосов", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты"), resource("Дрель", "Дрели")],
            [resource("Нож строительный", "Специализированные ножи"), resource("Шпатели", "Шпатели"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Подоконники и откосные системы"), proposedResource("Монтажная пена")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
        templateWork("Стены", [
          ["Покраска стен и потолков", resourceSet(
            [resource("Краскопульт", "Краскопульты"), resource("Краскораспылительный пистолет", "Краскораспылительные пистолеты")],
            [resource("Кисти малярные", "Кисти малярные"), resource("Валики и бугели", "Валики и бугели")],
            [proposedResource("Краска, грунтовка и малярный скотч")],
            [resource("Респиратор", "Респираторы"), resource("Защитные очки", "Очки защитные")],
          )],
          ["Поклейка обоев", resourceSet(
            [],
            [resource("Валики и бугели", "Валики и бугели"), resource("Шпатели", "Шпатели"), resource("Нож строительный", "Специализированные ножи")],
            [proposedResource("Обои и обойный клей")],
            [],
          )],
          ["Декоративная отделка", resourceSet(
            [resource("Краскопульт", "Краскопульты")],
            [resource("Шпатели", "Шпатели"), resource("Кисти малярные", "Кисти малярные")],
            [proposedResource("Декоративные составы, молдинги и багеты")],
            [resource("Респиратор", "Респираторы"), resource("Защитные очки", "Очки защитные")],
          )],
        ]),
      ]),
      templateStage(9, "Установка дверей, окон и напольного покрытия", [
        templateWork("Двери и окна", [
          ["Установка дверных коробок, полотен и наличников", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты"), resource("Перфоратор", "Перфораторы")],
            [resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Рулетка", "Рулетки"), resource("Нож строительный", "Специализированные ножи")],
            [resource("Буры", "Буры"), resource("Наборы бит", "Наборы бит"), proposedResource("Дверные блоки, наличники и монтажная пена")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
          ["Монтаж окон и откосов", resourceSet(
            [resource("Перфоратор", "Перфораторы"), resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Шпатели", "Шпатели"), resource("Нож строительный", "Специализированные ножи")],
            [proposedResource("Оконные блоки, подоконники и откосы"), proposedResource("Монтажная пена")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
        templateWork("Напольные покрытия", [
          ["Укладка ламината и паркета", resourceSet(
            [resource("Лобзик", "Лобзики электрические"), resource("Торцовочная пила", "Пилы торцовочные")],
            [resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Молоток", "Молотки")],
            [proposedResource("Ламинат, паркет и подложка"), resource("Пилки для электролобзика", "Пилки для электролобзиков")],
            [resource("Защитные очки", "Очки защитные")],
          )],
          ["Укладка плитки на пол", resourceSet(
            [proposedResource("Плиткорез"), resource("Дрель-миксер", "Дрели-миксеры")],
            [resource("Мастерки и кельмы", "Мастерки, кельмы, расшивки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Плитка и плиточный клей")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
          ["Монтаж плинтусов", resourceSet(
            [resource("Торцовочная пила", "Пилы торцовочные"), resource("Клеевой пистолет", "Клеевые пистолеты")],
            [proposedResource("Стусло"), resource("Рулетка", "Рулетки"), resource("Нож строительный", "Специализированные ножи")],
            [resource("Клеевые стержни", "Клеевые стержни"), proposedResource("Плинтусы и крепёж")],
            [resource("Защитные очки", "Очки защитные")],
          )],
        ]),
      ]),
      templateStage(10, "Финишные работы и сдача", [
        templateWork("Финишный монтаж инженерных систем", [
          ["Установка розеток, выключателей и светильников", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Отвёртка", "Отвертки"), resource("Пробник напряжения", "Пробники напряжения"), resource("Инструменты для обжима", "Инструменты для обжима")],
            [resource("Наборы бит", "Наборы бит")],
            [resource("Диэлектрические перчатки", "Перчатки")],
          )],
          ["Монтаж сантехники и бытовой техники", resourceSet(
            [resource("Дрель", "Дрели"), resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Ключ разводной", "Ключи разводные"), resource("Плоскогубцы", "Плоскогубцы и пассатижи"), resource("Отвёртка", "Отвертки")],
            [proposedResource("Унитазы, ванны, смесители и бытовая техника")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
        templateWork("Мебель и встроенные конструкции", [
          ["Сборка кухонного гарнитура и шкафов", resourceSet(
            [resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты"), resource("Дрель", "Дрели")],
            [resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Струбцина", "Струбцины")],
            [resource("Наборы бит", "Наборы бит"), resource("Сверла", "Сверла"), proposedResource("Мебельная фурнитура и крепёж")],
            [],
          )],
          ["Установка встроенных конструкций", resourceSet(
            [resource("Дрель", "Дрели"), resource("Лобзик", "Лобзики электрические")],
            [resource("Рулетка", "Рулетки"), resource("Уровень", "Уровни, угломеры и уклономеры")],
            [proposedResource("Встроенные конструкции и крепёж")],
            [],
          )],
        ]),
        templateWork("Уборка, проверка и сдача объекта", [
          ["Генеральная уборка", resourceSet(
            [resource("Мойка высокого давления", "Мойки высокого давления"), resource("Строительный пылесос", "Строительные пылесосы")],
            [],
            [resource("Оснастка для моек высокого давления", "Оснастка для моек высокого давления"), resource("Мешки для строительного пылесоса", "Мешки для строительных пылесосов"), resource("Фильтры для строительного пылесоса", "Фильтры для строительных пылесосов")],
            [resource("Защитные перчатки", "Перчатки"), resource("Респиратор", "Респираторы")],
          )],
          ["Проверка работы систем и устранение дефектов", resourceSet(
            [resource("Дрель", "Дрели"), resource("Шуруповёрт", "Шуруповёрты и дрели-шуруповёрты")],
            [resource("Мультиметр", "Мультиметры"), resource("Уровень", "Уровни, угломеры и уклономеры"), resource("Рулетка", "Рулетки"), resource("Отвёртка", "Отвертки")],
            [proposedResource("Оборудование для испытаний инженерных систем")],
            [resource("Защитные перчатки", "Перчатки")],
          )],
        ]),
      ]),
  ...
  function enrichWorkScenarios() {
    FULL.stages = buildTemplateStages();
    ...
    const resourceGroups = operation.resources
      ? resourceGroupDefs.map(({ key, title }) => {
          ...
          return `<article class="resource-group-card"><header><h2>${escapeHtml(title)}</h2><span>${items.length}</span></header><div class="resource-list">${items.map((item) => {
            ...
            const missingClass = item.proposed ? "resource-item missing proposed" : "resource-item missing";
            const missingLabel = item.proposed ? "Добавлено по шаблону — требуется сопоставление с ассортиментом" : "Требует сопоставления с ассортиментом";
            return `<div class="${missingClass}"><span><strong>${escapeHtml(item.name)}</strong>${meta ? `<small>${escapeHtml(meta)}</small>` : ""}<em>${missingLabel}</em></span><b>Нет ссылки</b></div>`;
          }).join("")}</div></article>`;
        }).join("")
      : "";
    const content = resourceGroups
      ? `<section class="resource-summary"><span>${operation.categoryIds.length} связанных категорий</span></section><section class="resource-group-grid">${resourceGroups}</section>`
